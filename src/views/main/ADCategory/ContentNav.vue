<template>
  <div>
      ...
      <el-dialog 
        ...
        <el-dialog 
            title="修改导航"
            :visible.sync="dialogUpdataNavVisible"
            width="30%"
            :before-close="handleClose"
        >
            <el-form :model="navUpdataForm" ref="navUpdataForm">
                <el-form-item label="导航名字">
                    <el-input type="text" v-model="navUpdataForm.name"></el-input>
                </el-form-item>
            </el-form>
            <span slot="footer" class="dialog-footer">
                <el-button @click="dialogUpdataNavVisible = false"
                    >取 消</el-button
                >
                <el-button type="primary" @click="updataContentNavHandle"
                    >确 定</el-button
                >
            </span>
        </el-dialog>
...
export default {
    ...
    methods:{
        ...
        http(id,resolve){
             this.$api.selectContentCategoryByParentId(id)
                    .then(res=>{
        ...
        return (
          <span class="custom-tree-node">
            ...
            <span>
              <el-button size="mini" type="text" on-click={ () => this.append(node,data) }>添加子分类</el-button>
      ...
      editor(node, data){
          ...
          this.dialogUpdataNavVisible = true;
      },
      // 确认添加
      addContentNavHandle(){
      ...
      // 修改的确认按钮
      updataContentNavHandle(){
          this.$api.updateContentCategory({
              pid:this.navContent.pid,
              name:this.navUpdataForm.name
          })
          .then(res=>{
              if(res.data.status===200){
                  this.reload();
                  this.dialogUpdataNavVisible=false
              }
          })
          .catch(err=>{
              console.log(err);
